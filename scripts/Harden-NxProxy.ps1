...
.DESCRIPTION
    Layer 1: Force DNS to 127.0.0.1 (NxProxy) on all active NICs
    ...
    Layer 4: Windows Firewall — allow only NxProxy.exe outbound DNS, block everything else
...
$nxProxySvc = Get-WmiObject Win32_Service -Filter "Name='$NxProxyServiceName'" -ErrorAction Stop
if (-not $nxProxySvc) {
    Write-Error "Service '$NxProxyServiceName' not found. Ensure NxProxy is installed before running this script."
    exit 1
}
$NxProxyExePath = $nxProxySvc.PathName -replace '"', ''
...
# =============================================================================
# LAYER 1 — Force DNS to NxProxy on all active NICs
# =============================================================================
Write-Host "`n=== LAYER 1: Forcing DNS to $DnsServer on all active adapters ===" -ForegroundColor Green
...
foreach ($adapter in $adapters) {
    ...
    # Disable DHCP DNS override by setting static DNS (already done above, but ensure no secondary)
...
foreach ($ifKey in $interfaceKeys) {
    ...
    try {
        ...
        # Remove existing inherited rules and explicit rules for Users/Everyone
...
if (-not (Test-Path $operaPolicyPath)) { New-Item -Path $operaPolicyPath -Force | Out-Null }
Set-ItemProperty -Path $operaPolicyPath -Name "DnsOverHttpsMode" -Value "off" -Type String
Write-Host "  Opera: DoH disabled"
...
# BLOCK: All other outbound DNS (TCP + UDP 53)
New-NetFirewallRule -DisplayName "$rulePrefix - Block All DNS (UDP)" `
    -Direction Outbound -Action Block -Protocol UDP -RemotePort 53 `
    -Enabled True -Profile Any | Out-Null

New-NetFirewallRule -DisplayName "$rulePrefix - Block All DNS (TCP)" `
    -Direction Outbound -Action Block -Protocol TCP -RemotePort 53 `
    -Enabled True -Profile Any | Out-Null

Write-Host "  Blocked: All other outbound DNS (TCP/UDP 53)"

# BLOCK: Known DoH provider IPs on port 443
$dohProviderIPs = @(
...
# Harden service DACL
# D: = DACL
# A;;RPWPCR;;;SY  = SYSTEM: all service permissions
# A;;RPWPCR;;;BA  = Administrators: all service permissions
# A;;CCLCSWLOCRRC;;;IU = Interactive Users: read + query status only (no stop/start)
# A;;CCLCSWLOCRRC;;;SU = Service Users: read + query status only
$dacl = 'D:(A;;RPWPCR;;;SY)(A;;RPWPCR;;;BA)(A;;CCLCSWLOCRRC;;;IU)(A;;CCLCSWLOCRRC;;;SU)'
& sc.exe sdset $NxProxyServiceName $dacl | Out-Null
Write-Host "  Service DACL hardened: standard users cannot stop/start/modify"
...
<AppLockerPolicy Version="1">
  <RuleCollection Type="Exe" EnforcementMode="Enabled">
    <!-- Default: Allow Administrators everything -->
    ...
    <!-- Default: Allow Users to run from Windows directory -->
    ...
    <!-- Default: Allow Users to run from Program Files -->
    ...
    <!-- Allow Users: WindowsApps (for any remaining Store apps / system components) -->
  ...
  <RuleCollection Type="Msi" EnforcementMode="Enabled">
    <!-- Default: Allow Administrators all MSI -->
    ...
    <!-- Default: Allow Users MSI from Windows Installer -->
...
Write-Host "  Layer 4: Firewall (allow NxProxy, block all other)   [DONE]" -ForegroundColor Green
